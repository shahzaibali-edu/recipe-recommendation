...
**Stack:** Python, PyTorch, EfficientNet-B0, Spoonacular API, Streamlit  
**Status:** Model trained, evaluated and saved; UI and API Pipeline Integrated ✅  
...
## Step 9 — API Integration (`app.py`) ✅

**API Setup:**
- Integrated the **Spoonacular API** for generating recipe recommendations based on identified ingredients.
- Spoonacular provides comprehensive recipe data including titles, images, ingredient lists, instructions, and nutritional information.

**Security Implementation:**
- Created a `.env` file to securely store the `SPOONACULAR_API_KEY`.
- Added `.env` to `.gitignore` to prevent leaking credentials to version control.
- Used `python-dotenv` library to load environment variables safely.

**API Endpoint Used:**
- Endpoint: `GET /recipes/findByIngredients`
- Parameters:
  - `ingredients`: Comma-separated list of ingredient names
  - `number`: Number of recipes to return (default: 5)
  - `ranking`: Maximize used ingredients (set to 1)
  - `ignorePantry`: Exclude common pantry items from matching (set to true)
...
↓
Model inference (EfficientNet-B0 forward pass)
↓
Get predicted class (argmax on logits)
↓
Map class index to ingredient name (e.g., index 32 → "tomato")
↓
Send ingredient name to Spoonacular API
↓
Parse API response (extract recipe titles, images, ingredient counts)
↓
Display results to user
```

**Implementation Details:**
- **Model Loading:** Load the saved `best_model.pth` checkpoint with the trained EfficientNet-B0 weights.
- **Class Mapping:** Extract the `class_names` list saved within the model checkpoint to map prediction indices to ingredient labels.
- **Preprocessing:** Apply the same transforms used during validation (resize + normalize, no augmentation).
- **Prediction:** Run forward pass in evaluation mode (`model.eval()`) with `torch.no_grad()` for efficiency.
- **Confidence Thresholding:** Only send ingredient to API if prediction confidence exceeds a minimum threshold (e.g., 80%).

**Error Handling:**
- Handle corrupted/invalid image uploads gracefully.
- Catch API rate limit errors and display user-friendly messages.
- Validate API responses before parsing to prevent crashes.

---

## Step 11 — UI Development (`app.py`) ✅

**UI Framework:** Deployed a local frontend using **Streamlit**.

**Features Implemented:**
- **Image Upload Widget:** Users can upload an image in `.jpg`, `.jpeg`, or `.png` format.
- **Image Display:** Uploaded image is displayed on the interface for user confirmation.
- **Prediction Display:** Shows the predicted ingredient name along with confidence percentage.
- **Recipe Cards:** Displays top N recipes with:
  - Recipe title (clickable link to full recipe)
  - Recipe image thumbnail
  - Number of matching ingredients used
  - Number of missing ingredients needed
- **Loading State:** Shows a spinner while the model processes the image and fetches recipes.
- **Error Messages:** User-friendly error handling for invalid uploads or API failures.
...
│                                         │
│  Predicted Ingredient: Tomato (98.5%)   │
├─────────────────────────────────────────┤
...
| 12 | Streamlit failed to locate `app.py` initially | Moved `app.py` from the `notebooks/` sub-directory into the main project root directory |

---

## Future Scopes (Phase 2 & Enhancements)

- **Mobile Camera Integration:** Upgrade the Streamlit UI using `st.camera_input` to allow users to capture pictures directly from their phone/webcam instead of just uploading files.
- **YOLOv8 Multi-Ingredient Detection:** Transition from the single-class EfficientNet-B0 model to a YOLO-based object detection architecture. This will allow the system to process an image of an entire cutting board, detect multiple distinct ingredients (e.g., tomatoes, onions, and chicken) simultaneously, and pass a combined array of ingredients to the Spoonacular API for highly specific recipe generation.
- **Nutritional Information Display:** Add calorie counts, macronutrients, and dietary tags (vegetarian, vegan, gluten-free) for each recommended recipe.
- **User Preferences:** Allow users to filter recipes by cuisine type, cooking time, difficulty level, or dietary restrictions.
- **Recipe Saving:** Implement a feature for users to save favorite recipes to a personal collection.
- **Ingredient Quantity Detection:** Combine computer vision with weight/volume estimation to suggest recipes based on available ingredient quantities.
- **Cloud Deployment:** Deploy the application to Streamlit Cloud, Heroku, or AWS for public access.
...
│
├── app.py                          ← Streamlit UI and Spoonacular API inference pipeline
...
| 9 | Connect Spoonacular API — send ingredient → get recipes | ✅ Done |
| 10 | Build inference pipeline — photo in, recipe list out | ✅ Done |
| 11 | Simple UI — upload photo, see recipes | ✅ Done |
...
- Spoonacular API key required — sign up for free tier at [spoonacular.com/food-api](https://spoonacular.com/food-api)
- Free tier API limits: 150 requests/day

---

*Log last updated after UI Development & API Integration*
